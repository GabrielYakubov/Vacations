...
export const downloadCsv = async (): Promise<void> => {
  try {
    const response = await axios.get<Blob>(`${BASE_API_URL}/followers/csv`, {
      responseType: "blob",
    });
...
export const addLike = async (
  ...
  const response = await axios.post<Like>(`${BASE_API_URL}/likes/`, {
    vacationId,
    userId,
  });
...
export const updateVacation = async (vacation: Vacation): Promise<Vacation> => {
  ...
  const response = await axios.put<Vacation>(
    `${BASE_API_URL}/vacation/${vacation.vacationId}`,
    formData
  );
